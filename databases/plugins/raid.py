...
from resources.data import RAID, REPLYRAID, ADITYASPAM
...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(Aditya) == 2:
            ...
            g = a.id
            if int(g) in Aditya:
                text = f"I can't raid on Kaal Spam Bot Owner"
...
async def _(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 11:
            message = str(Aditya[0])
            a = await e.client.get_entity(message)
            user_idd = a.id
            user_id = int(user_idd)
            if int(user_id) in Aditya:
                text = f" can't raid on Kaal Spam Bot Owner Owner."
                await e.reply(text, parse_mode=None, link_preview=None )
            ...
            user_idd = umser.id
            user_id = int(user_idd)
            if int(user_id) in ADITYASPAM:
                text = f" can't raid on Kaal Spam Bot Owner."
                await e.reply(text, parse_mode=None, link_preview=None )
            elif int(user_id) == OWNER_ID:
                text = f"This guy is a owner Of this Bots."
                await event.reply(text, parse_mode=None, link_preview=None )
...
async def _(event):
   ...
   if event.sender_id in SUDO_USERS:
         if event.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage, parse_mode=None, link_preview=None )
         Aditya = "".join(event.text.split(maxsplit=1)[1:]).split(" ", 2)
         if len(Aditya) == 3:
             user = str(Aditya[2])
             a = await event.client.get_entity(user)
             e = a.id
             if int(e) in ADITYASPAM:
                    text = f"I can't raid on Kaal Spam Bot Owner"
                    await event.reply(text, parse_mode=None, link_preview=None )
             elif int(e) == OWNER_ID:
                text = f"This guy is a owner Of this Bots."
                await event.reply(text, parse_mode=None, link_preview=None )
                 ...
                 counter = int(Aditya[1])
                 sleeptimet = sleeptimem = float(Aditya[0])
                 for _ in range(counter):
                      reply = random.choice(RAID)
                      caption = f"{username} {reply}"
                      async with event.client.action(event.chat_id, "typing"):
