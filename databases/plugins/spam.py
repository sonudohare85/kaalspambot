...
from databases import BOT0, BOT1, BOT2, BOT3, BOT4, BOT5, BOT6, BOT7, BOT8, BOT9, SUDO_USERS, OWNER_ID
...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        ...
        Aditya = ("".join(e.text.split(maxsplit=1)[1:])).split(" ", 1)
        smex = await e.get_reply_message()
        if len(Aditya) == 2:
            message = str(Aditya[1])
            counter = int(Aditya[0])
            if counter > 100:
...
async def spam(e):
    ...
    if e.sender_id in SUDO_USERS:
        if e.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage, parse_mode=None, link_preview=None)
        smex = await e.get_reply_message()
        Aditya = "".join(e.text.split(maxsplit=1)[1:]).split(" ", 2)
        Adityasexy = Aditya[1:]
        if len(Adityasexy) == 2:
            message = str(Adityasexy[1])
            counter = int(Adityasexy[0])
            sleeptime = float(Aditya[0])
            for _ in range(counter):
                async with e.client.action(e.chat_id, "typing"):
                    if e.reply_to_msg_id:
                        await smex.reply(message)
...
async def unlimitedspam(event):
  if e.sender_id in SUDO_USERS:
    try:
      ...
      await event.reply("Oops!! Something went wrong, Report In @IAMKAAL\n\n" + str(e))


@BOT0.on(events.NewMessage(incoming=True, pattern=r"\%skaalspam(?: |$)(.*)" % hl))
@BOT1.on(events.NewMessage(incoming=True, pattern=r"\%skaalspam(?: |$)(.*)" % hl))
@BOT2.on(events.NewMessage(incoming=True, pattern=r"\%skaalspam(?: |$)(.*)" % hl))
@BOT3.on(events.NewMessage(incoming=True, pattern=r"\%skaalspam(?: |$)(.*)" % hl))
@BOT4.on(events.NewMessage(incoming=True, pattern=r"\%skaalspam(?: |$)(.*)" % hl))
@BOT5.on(events.NewMessage(incoming=True, pattern=r"\%skaalspam(?: |$)(.*)" % hl))
@BOT6.on(events.NewMessage(incoming=True, pattern=r"\%skaalspam(?: |$)(.*)" % hl))
@BOT7.on(events.NewMessage(incoming=True, pattern=r"\%skaalspam(?: |$)(.*)" % hl))
@BOT8.on(events.NewMessage(incoming=True, pattern=r"\%skaalspam(?: |$)(.*)" % hl))
@BOT9.on(events.NewMessage(incoming=True, pattern=r"\%skaalspam(?: |$)(.*)" % hl))
async def pspam(e):
    if e.sender_id in SUDO_USERS:
        if e.text[0].isalpha() and e.text[0] in ("/", "#", "@", "!"):
            return await e.reply(usage, parse_mode=None, link_preview=None )
